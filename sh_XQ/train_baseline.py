...
version = 1
# tag = '5classbalance'
for tag in ['5classbalance', '7classbalance']:
    dataset_tag = f'XQ{tag}'
    cfg_tag = f'-{tag}'
    for source_dataset in ["XQpink", "XQXY"]:
        ...
        for version in [1]:  
            ...
            subprocess.run(f"python {sys.path[0]}/test.py --source_dataset={source_dataset} --target_dataset {target_dataset} "
                        f"--config_dir={config_dir} --dataset_tag={dataset_tag} --work_dir={work_dir} --gpu={GPUS[0]}", shell=True)
