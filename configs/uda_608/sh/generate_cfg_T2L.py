...
for T in T_list:
    for a in a_list:
        for cfg in cfg_list:
            # name
            new_cfg_name = f'uda_yolov3-{dataset}-SCL-Xohem-{DA}-cfav{cfg}-{cfa_weight}-T{T}-a{a}{fp16}.py'
            
            print(new_cfg_name)
            with open(template_path, "r") as f:
                srcCfg = f.read().split("\n")
                # fp16
                srcCfg[1-1] = f"_base_ = [  '../../../yolo/uda_yolov3_608_{dataset}_SCL-100e-000-0{fp16}.py', ]"
                # GFA weight
