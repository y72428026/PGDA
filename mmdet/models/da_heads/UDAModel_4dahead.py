...
class UDAModel(SingleStageDetector):
    ...
    def extract_feat_mine(self, img):
        ...
        xb = self.get_model().backbone(img)
        if self.get_model().with_neck:
            xn = self.get_model().neck(xb)
        print([ele.shape for ele in xb])
        print([ele.shape for ele in xn])
        input("?")
        return xb, xn 
    ...
    def forward_train(self,
        ...
        feat_bone, feat = self.extract_feat_mine(target_img)
        trg_da_b_loss = add_prefix(self.da_backbone_head(feat_bone[-3:], is_source=False),'trg_b')
        trg_da_n_loss = add_prefix(self.da_head(feat[-3:], is_source=False),'trg_n')
        losses.update(trg_da_b_loss)
        losses.update(trg_da_n_loss)
        feat_maps, pred_maps = model.bbox_head.return_feat_pred(feat)
        trg_da_h_loss = add_prefix(self.da_ano_head(feat_maps, is_source=False), 'trg_h')
        # trg_da_pred_loss = add_prefix(self.da_pred_head(pred_maps, is_source=False), 'trg_pred')
        losses.update(trg_da_h_loss)
        ...
        feat_bone, feat = self.extract_feat_mine(target_img)
        src_da_b_loss = add_prefix(self.da_backbone_head(feat_bone[-3:], is_source=True),'src_b')
        src_da_n_loss = add_prefix(self.da_head(feat[-3:], is_source=True),'src_n')
        losses.update(src_da_b_loss)
        losses.update(src_da_n_loss)
        feat_maps, pred_maps = model.bbox_head.return_feat_pred(feat)
        src_da_h_loss = add_prefix(self.da_ano_head(feat_maps, is_source=True), 'src_h')
        # src_da_pred_loss = add_prefix(self.da_pred_head(pred_maps, is_source=True), 'src_pred')
        losses.update(src_da_h_loss)
